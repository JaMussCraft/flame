...
def results_to_dataframe(results_dict: Dict) -> pd.DataFrame:
    ...
    for key, experiment_results in results_dict.items():
        print("YIPPIE: ", experiment_results)
...
def analyze_final_results(df: pd.DataFrame) -> pd.DataFrame:
    ...
    final_results = df.groupby(['world_size', 'learning_rate', 'pretrain', 'enable_swapping', 'rounds', 'seed']).last()
    
    # Calculate statistics
    stats = final_results.groupby(['world_size', 'learning_rate', 'pretrain', 'enable_swapping', 'rounds']).agg({
...
def plot_hyperparameter_comparison(df: pd.DataFrame, save_plots: bool = True):
    ...
    final_df = df.groupby(['world_size', 'learning_rate', 'pretrain', 'enable_swapping', 'rounds', 'seed']).last()
    ...
    # Pretrain comparison
    sns.boxplot(data=final_df.reset_index(), x='pretrain', y='test_accuracy', ax=axes[1,0])
    axes[1,0].set_title('Final Accuracy vs Pretrain')
    axes[1,0].set_xlabel('Pretrain')
    axes[1,0].set_ylabel('Test Accuracy')
    
    # Enable swapping comparison
    sns.boxplot(data=final_df.reset_index(), x='enable_swapping', y='test_accuracy', ax=axes[1,1])
    ...
    plt.tight_layout()
    if save_plots:
        plt.savefig('hyperparameter_comparison_resnet34.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def print_best_configurations(df: pd.DataFrame, top_n: int = 5):
    ...
    config_means = final_df.reset_index().groupby(['world_size', 'learning_rate', 'pretrain', 'enable_swapping', 'rounds'])['test_accuracy'].mean().sort_values(ascending=False)
    ...
    for i, (config, accuracy) in enumerate(config_means.head(top_n).items()):
        ...
        print(f"{i+1}. WS={world_size}, LR={learning_rate}, Pretrain={pretrain}, Swap={enable_swapping}, Rounds={rounds}: {accuracy:.4f}")
...
def plot_specific_experiments(df: pd.DataFrame, 
    ...
    unique_configs = filtered_df[['world_size', 'learning_rate', 'pretrain', 'enable_swapping', 'rounds']].drop_duplicates()
    
    print(f"Found {len(unique_configs)} unique configurations matching criteria:")
    for _, config in unique_configs.iterrows():
        print(f"  WS={config['world_size']}, LR={config['learning_rate']}, Pretrain={config['pretrain']}, Swap={config['enable_swapping']}, Rounds={config['rounds']}")
    ...
    for i, (_, config) in enumerate(unique_configs.iterrows()):
        ...
        plt.plot(avg_data.index, avg_data.values, label=label, marker=marker, 
    ...
    if save_plots:
        ...
        safe_filename = "".join(c for c in plot_title if c.isalnum() or c in (' ', '-', '_')).rstrip()
...
def plot_experiments_interactive():
    ...
    plot_specific_experiments(df, world_sizes, learning_rates, pretrain, enable_swapping,
                            rounds_list, seeds, save_plots=True, plot_title=plot_title)

def main():
    """Main function for the analysis script."""
    
    parser = argparse.ArgumentParser(description='Analyze CIFAR100 ResNet34 experiment results')
    parser.add_argument('--results-file', default='aggregator/experiment_results.pkl', 
